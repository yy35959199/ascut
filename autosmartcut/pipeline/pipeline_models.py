...
class NodeState:
    ...
    节点的持久化状态记录在 manifest["layer_status"][node_id] 中，采用三态模型：
    
    1. 无记录 → "never_started"：节点从未执行过
    2. 仅 started_at → "started"：节点开始执行但被中断（OOM/SIGKILL/crash）
    3. started_at + failed_at → "failed"：节点显式失败（LLM 错误等）
    4. started_at + completed_at → "completed"：节点成功完成
    
    此 NodeState 是运行时快照，用于 Scheduler 做决策；
    持久化状态由 manifest_io 的 ls_* 函数管理。
    """
