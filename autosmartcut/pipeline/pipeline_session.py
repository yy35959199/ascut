...
from autosmartcut.pipeline.pipeline_events import (
    ErrorEvent,
    PausedEvent,
...
from autosmartcut.pipeline.pipeline_protocols import (
...
class PipelineSession:
        ...
        while not self._abort_flag:
            ...
            for task in done_tasks:
                ...
                try:
                    ...
                    from autosmartcut.pipeline.pipeline_models import StageResult, StageStatus
                    result = StageResult(
    ...
    async def _run_node(
        ...
        # l2d_human 节点走交互路径（_handle_interactive 内部负责 started_at 落盘）
        ...
        # 立即写 started_at 并落盘：时间窗口最小化，硬中断后可推断中断状态
        ls_mark_started(manifest, node_id)
    ...
    def _mark_node_completed(self, node_id: str, manifest: dict) -> None:
        ...
        ls_mark_completed(manifest, node_id)
    ...
    def _mark_node_failed(self, node_id: str, result: StageResult, manifest: dict) -> None:
        """FAILED：写失败信息到 layer_status 并落盘，发 ErrorEvent，置中止标志。"""
        self._node_states[node_id].status = "failed"
        ls_mark_failed(manifest, node_id, result.error, result.summary)
        self._persist_manifest(manifest)
    ...
    async def _handle_result(
        ...
        if result.status == StageStatus.SUCCESS:
            ...
            self._mark_node_failed(node_id, result, manifest)
