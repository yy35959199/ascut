...
def run_l1b_align_only(
	...
	*,
	...
	spans = segment_raw_text_only(raw_text, sentence_endings, max_c)
	validate_index_text_against_spans(annotations, spans)
	...
	if len(timings) != len(annotations):
		raise ValueError("句级时间条数与 annotations 不一致")

	video_path = Path(str(data.get("source") or run.video_path))
	...
	for ann, tm in zip(annotations, timings):
	...
	data["annotations"] = compact_annotations(annotations)
