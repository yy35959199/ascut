...
if _TEXTUAL_AVAILABLE:
    ...
    class ResumeScreen(Screen):
        ...
        def _update_button_states(self) -> None:
            ...
            # L2 子阶段 RadioButton 可用性（基于 resumable_from，按索引顺序）
            # RadioSet 子项顺序：0=2a, 1=2b, 2=2c, 3=2d
            resumable_from = getattr(report, "resumable_from", {})
            _index_to_short = {0: "2a", 1: "2b", 2: "2c", 3: "2d"}
            try:
                radio_set = self.query_one("#l2-radio-set", RadioSet)
                for idx, short_name in _index_to_short.items():
                    enabled = resumable_from.get(short_name, False)
                    buttons = list(radio_set.query(RadioButton))
                    if idx < len(buttons):
                        buttons[idx].disabled = not enabled
            except Exception:
                pass
