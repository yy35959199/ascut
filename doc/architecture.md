...
默认 `register_default_nodes()` 注册 **6 个节点**，通过 `reads`/`writes` 字段自动推导依赖关系（L1→L2→L3 线性，无 L3 预计算分支）：

| node_id | reads | writes | phase | resumable |
|---------|-------|--------|-------|-----------|
| `l1_perception` | `source_media` | `annotations`、`raw_text` 等 | 1 | False |
...
**调度顺序（示意）：** `l1_perception` → `l2a_comprehension` → `l2b_decision` → `l2c_review` → `l2d_human` → `l3_execute`。`l3_execute` 通过读取 `l2d_completed` 与 `l2d_human` 建立 DAG 边。
...
| `l3_segment_count` | int | L3 执行后写入的保留段时间段数量（供摘要展示） |
...
算法核心（不依赖 PipelineRun，不感知 CLI）：
  ...
  execution.py               # L3 执行层（keep_mask → smartcut）
