...
9. [核心风险与应对](#9-核心风险与应对)
10. [一句话总结](#10-一句话总结)
11. [单一 TimelineManifest 文件架构（目标形态）](#11-单一-timelinemanifest-文件架构目标形态)
...
| 语义片段 | Segment | **完整版愿景**：2b 产出的、带语义标签与权重的时间单元。**MVP**：主交付为逐句 **`keep_mask`**（与内存 **`tokens[]`** / 清单 **`annotations[]`** 等长），不实现 `segments[]` 路径 |
...
| 识别层 | Layer 1 / Perception | 三层架构的第一层；现行工程可拆分为 **L1A（ASR 文本定稿）** 与 **L1B（强制对齐补时间）**，共同写入同一份标注清单 |
...
| 执行层 | Layer 3 / Execution | 三层架构的末层：由 `keep_mask` 与清单 **`annotations[]`** 合成保留时间区间，**MVP 可选**经 Silero VAD 做**切点吸附（Snap）**，再由 **smartcut** 执行 GOP 级精确剪切 |
...
| 消歧标注 | Cleaned Annotations | 在不修改原始句面的前提下写入的校正文本视图：全链一致时对应清单 **`annotations[].content`**，智能层侧对应内存 **`tokens[].text`**。**MVP**：由 **程序**根据 2a R2 的 `corrections` 生成稠密 `cleaned_annotations[]`（默认不落盘），非 LLM 直接输出整表 |
...
> **MVP 现行（工程契约）**：当前仓库以 **单一 `timeline_manifest.json`** 为层间主文件（`annotations[]`、`current.comprehension`、`current.keep_mask` 等）；2a（**R1 单轮 + R2 与 R1 同一对话前缀上的第二跳**，`autosmartcut.intelligence_llm` 多轮 API，见 [intelligence-layer2-mvp.md](intelligence-layer2-mvp.md) §5.1.1）+ **程序**生成内存 `cleaned_annotations`、智能层交付 **`current.keep_mask`**（**不**在智能层落盘 `edl[]`，由执行层合成剪切区间）；`tokens[]` **仅内存**、由 `annotation_tokens.tokens_from_annotations` 派生。细节以 [intelligence-layer2-mvp.md](intelligence-layer2-mvp.md)、[AutoSmartCut-MVP.md](AutoSmartCut-MVP.md)、[AutoSmartCut-MVP-Mini.md](AutoSmartCut-MVP-Mini.md) 为准。  
> 下列树状结构描述**愿景形态**：含 checklist、`segments[]`、清单内 `edl[]` 等，用于长期架构对齐，**勿**与 MVP 代码逐项等同。  
> **目标形态（单一持久化文件）**：以 `timeline_manifest.json` 为锚、含 `current` / `previous` 轮次快照与历史裁剪策略的完整设计，见 [§11](#11-单一-timelinemanifest-文件架构目标形态)。
...
三层管道按时间轴语义逐级收敛、单向流转：**识别层（Layer 1）**负责把原始媒体转为结构化标注。现行工程中，Layer 1 可拆分为 **L1A（ASR 文本定稿）** 与 **L1B（强制对齐补时间）**：L1A 先冻结 `index-text` 供下游立即消费，L1B 只回填句级 `t_start`/`t_end`/`gap_after`，且不改 `index/content`。二者共同写入清单 **`annotations[]`**。**智能层（Layer 2）**是唯一与 LLM 交互的中枢，按「2a 理解 → 2b 决策 → 2c 审核 → 2d 人工」闭环运行（完整版含检查清单、片段级覆盖报告与清单内 EDL 等叙事）。**MVP 收窄**：2a 为 **R1 单轮 + R2 真多轮第二跳** + 程序替换生成内存 `cleaned_annotations`（不持久化符号表）；2b 产出与内存 **`tokens[]`**（由 `annotations[]` 派生）索引对齐的 **`keep_mask`**，落盘为 **`current.keep_mask`**；**执行层**根据 `keep_mask` 与同文件 **`annotations[]`** 的时间信息**内部**得到剪切区间并驱动 smartcut，详见 [intelligence-layer2-mvp.md](intelligence-layer2-mvp.md) §5–§11。**执行层（Layer 3）**按 GOP（Group of Pictures，画面组）级精确剪切策略完成输出，绝大多数区间直接搬运原始比特流，仅在切点邻域局部重编码，从而兼顾速度、精度与画质。
...
2. **L1A 语音转写（文本定稿）**：将 WAV 送入 Qwen3-ASR-1.7B，先产出段落级转写文本，并按统一标点规则切分为稳定的句级 `index-text`（不含句级时间）。

3. **L1B 强制对齐（时间补丁）**：用 Qwen3-ForcedAligner-0.6B 产出字/词级时间戳，并映射回 L1A 已冻结的句级 span，仅回填 `t_start`/`t_end`（不改 `index/content`）。

4. **句间间隔（gap_after）**：在 L1B 回填时间后，对相邻句级片段用「下一句 `t_start` − 当前句 `t_end`」得到间隔；最后一句用「媒体时长 − 当前句 `t_end`」作为尾部间隔。该值为非负浮点秒数，写入当前句的 `gap_after` 字段，**不**再插入独立的静音标注行。

5. **写入清单与状态**：将句级标注写入 `TimelineManifest.annotations[]`；L1A/L1B 分别可写 `layer_status.l1a_completed_at` / `layer_status.l1b_completed_at`，完整 L1 可写 `layer_status.l1_completed_at`。
...
**输入（MVP 现行）：** 从 **`timeline_manifest.json`** 读取 **`annotations[]`**；在内存中生成 **`tokens[]`**（`index` + `text`，与 `annotations` **逐条对齐**）+ 用户通过 `--goal` 指定的分析目标。LLM Prompt 的句面来自内存 `tokens`；**时间轴字段**（`t_start`/`t_end`/`gap_after`）在同文件的 `annotations[]` 中，**不**进入 2a/2b 的 LLM 文本，由 **Layer 3** 与执行层纯函数消费。

**输入（完整版愿景）：** 识别层产出的 `annotations[]`（或等价句面）+ `--goal`

**输出（完整版愿景）：** 语义片段列表（`segments[]`）、剪辑决策表（`edl[]`）、理解结果（`comprehension`）、审核报告列表（`review_reports[]`）

**输出（MVP 现行）：** 写回清单 **`current.comprehension`**（`purpose`、`outline_blocks` 等）与 **`current.keep_mask`**（与 [intelligence-layer2-mvp.md](intelligence-layer2-mvp.md) 一致）；**不**在智能层持久化 `edl[]`。
...
**MVP 现行：** LLM 句面为内存 **`tokens[]`**（仅 `index` + `text`，由清单 `annotations[]` 派生）+ `goal`。**R1** 一次结构化调用；**R2** 与 R1 为 **OpenAI 兼容 API 下的真多轮**（`assistant` 消息承载 R1 的 JSON 字符串，再发 R2 `user`），实现见 `autosmartcut.intelligence_llm`（`call_once_structured_with_raw_content`、`prepare_next_turn_messages`、`call_turn_structured`）。R1（内存）：`purpose_rough`、`outline_blocks_rough`、`candidate_misrecognitions`。R2（内存）：`purpose`、`outline_blocks`、`corrections`。随后**程序**按 `corrections` 在 **`tokens[].text`** 上生成稠密 **`cleaned_annotations[]`**，**不**改写清单内 `annotations[].content`，**不**修改已加载的 `tokens[]` 原文。`cleaned_annotations` 等运行时键在保存前按 `manifest_io.strip_volatile_fields` 处理。详见 [intelligence-layer2-mvp.md](intelligence-layer2-mvp.md) §5、§5.1.1。
...
- **第 2 轮（印证）**：输入原始文本 + 粗糙主旨 + 符号表，产出精化主旨、`cleaned_annotations[]`（**MVP 中改为程序根据 `corrections` 生成**）与最终 `checklist[]`。其中消歧文本只追加写入，不覆盖 Layer 1 的原始句面（`annotations[].content` / 内存 `tokens[].text`），因此仍然遵循 Append-only。
...
**输入（MVP）：** `comprehension.purpose`、`outline_blocks`、`cleaned_annotations`（稠密全量，未纠错句与 **`tokens[].text`** 一致）+ 内存 **`tokens[]`**（用于构造 Prompt）。**不**把 **`gap_after`** 与起止时间塞进 LLM 文本；时间字段留在清单 **`annotations[]`**，由执行层消费。

**输入（完整版）：** 2a 产出的检查清单 + `cleaned_annotations[]` + 句级 `annotations[]`（含 `gap_after`）

**LLM 做什么（MVP）：** 对每个句级 `index` 输出 **`keep: true|false`**，写入 **`keep_mask[]`**（与 **`len(tokens)`** 等长，亦即与 `annotations[]` 条数相同）；可选预留 `checklist_coverage[]`（可空）。落盘位置为清单 **`current.keep_mask`**。
...
**输入（MVP 现行）：** 同一份 **`timeline_manifest.json`** 中的句级 **`annotations[]`**（含 `t_start`/`t_end`/`gap_after`）+ **`current.keep_mask[]`**。执行层在内部将连续 `keep=true` 的句合并为保留区间，再转为 `Fraction` 列表驱动 smartcut；**不**依赖智能层落盘 `edl[]`。参见 [intelligence-layer2-mvp.md](intelligence-layer2-mvp.md) §10–§11。
...
1. **得到 `Fraction` 时间区间**：
   - **MVP**：由 `keep_mask` 与 **`annotations[]`** 的时间–index 映射得到浮点保留区间，经 `gap_after_cap`、`pre_pad`/`post_pad` 整形；**可选**从源视频再解码 16 kHz 单声道音轨（仅内存），用 **Silero VAD** 得到静音区间，对入点/出点做 **Snap（切点吸附）** 后再合并重叠区间并 clamp 到 `[0, duration]`；最后经 `min_duration` 等规则合并，转为 `Fraction`（有理数精确表示），避免浮点误差积累。配置与 CLI 开关见 [AutoSmartCut-MVP.md](AutoSmartCut-MVP.md)。
...
> 以**时间轴清单（TimelineManifest）**为核心资产，分三层单向流转：**识别层**把原始媒体转化为标注，**智能层**通过四子阶段（理解→决策→审核→人工）把标注转化为决策与语义理解（完整版含循环与清单内 EDL 叙事），**执行层**把保留区间渲染为最终产物。**MVP** 下持久化为 **`timeline_manifest.json`**；智能层以内存 **`tokens[]`**（由 `annotations[]` 派生）为句面输入，2a 采用 **真多轮 LLM**（R1→R2）+ 程序消歧，对外写回 **`current.keep_mask`**，剪切区间在执行层合成，并可选用 **VAD 切点吸附（Snap）** 改善接缝听感。每层产物只增不改、可持久化、可重跑；扩展能力通过节点注册实现，不修改主干。视频剪辑是第一个应用，架构的射程是一切时间轴媒体的语义化。

---

## 11. 单一 TimelineManifest 文件架构（目标形态）

> **工程现状**：当前仓库 **已实现** 以 **`timeline_manifest.json`** 为层间主持久化文件（MVP-mini / `ascut run --manifest`）；`autosmartcut.manifest` 中的 `TimelineManifest` dataclass 仍为完整版愿景的类型草图。  
> **本节**：在「单文件清单」前提下描述**完整版目标架构**（多轮 `current`/`previous`、`history_summary` 等），与 §3 树状愿景一致；MVP 缩略形态见 [§11.9](#119-mvp-mini透传-2c2d-的缩略形态供对照) 与 [AutoSmartCut-MVP-Mini.md](AutoSmartCut-MVP-Mini.md)。

### 11.1 设计目标

1. **单一入口**：编排与各层 I/O 以同一份清单文件为锚，减少「多 JSON 路径约定」带来的心智负担与校验分叉。
2. **源数据不可变**：`annotations[]` 为识别层原始事实，后续阶段**只读**，不覆写句面与字级时间戳。
3. **当前轮为 checkpoint**：智能层多轮循环下，每一轮结束时的**物化状态**必须可独立作为下一轮的输入基线；不能假设「永远可从最初版本 + 全量历史 corrections 重算到当前」——因为历史会被裁剪。
4. **历史可裁剪**：磁盘上只长期保留 **当前轮（`current`）** 与 **上一轮（`previous`）** 的完整快照；更早轮次仅保留 **精简摘要**（审计与成本平衡）。
5. **零成本派生不落盘**：`tokens[]`（句面投影）仅为 `annotations[].{index, content}` 的**纯形式投影**，O(n) 微秒级，**不写入清单**；需要时由纯函数从 `annotations[]` 生成。

### 11.2 顶层结构（清单文件）

建议文件名：`timeline_manifest.json`（具体命名以工程约定为准）。顶层字段：

| 字段 | 含义 |
|------|------|
| `version` | 清单 schema 版本号，供迁移与兼容。 |
| `run_id` | 单次流水线运行标识（如 ULID）。 |
| `goal` | 用户传入智能层的剪辑/分析目标。 |
| `source_media` | 源视频引用：`path`、`duration` 等；**不包含**媒体二进制。 |
| `annotations[]` | Layer 1 句级标注（含 `index`、`t_start`、`t_end`、`content`、`gap_after`、`confidence`、`metadata` 等）。 |
| `current` | **当前轮**完整快照（见 §11.3）。 |
| `previous` | **上一轮**完整快照（结构同 `current`）；无循环或首轮可为空对象或省略。 |
| `history_summary[]` | 早于 `previous` 的轮次**仅摘要**（append-only）；见 §11.5。 |
| `layer_status` | 各宏观层完成时间戳等元信息（如 `l1a_completed_at`、`l1b_completed_at`、`l1_completed_at`、`l2_completed_at`、`l3_completed_at`），便于断点续跑与观测。 |

**与 §3 愿景树的关系**：`comprehension`、`keep_mask`、`review_reports`、`human_feedback_history`、`edl[]`、`loop_metadata` 等叙事在目标形态中**主要落在 `current` / `previous` 的快照内**（或顶层轻量字段），避免与「单文件」原则冲突；清单仍保持**可序列化 JSON**。

### 11.3 `current` / `previous` 快照结构（完整版）

`current` 与 `previous` 结构相同，表示某一智能层轮次结束时的**一致状态**（写入顺序由实现保证原子性或最后写 `current`）：

| 子字段 | 含义 |
|--------|------|
| `round` | 智能层轮次编号（从 1 递增）。 |
| `timestamp` | 该快照写入时间（ISO8601）。 |
| `comprehension` | 2a 产出：`purpose`、`outline_blocks[]`、`corrections[]`（错词替换 delta）；完整版可含 `checklist[]`、`symbol_table[]` 等。 |
| `cleaned_annotations[]` | **稠密全量**消歧句面（`annotation_index` + `cleaned_content`）。在**历史 corrections 被裁剪**或**多轮 corrections 为链式依赖**时，必须从上一轮物化结果延续，**必须落盘于快照**，不能仅靠「源 annotations + 仅存最新 corrections」在无全量历史时唯一重建。 |
| `keep_mask[]` | 2b 草案：与句级 `index` 对齐的 `{ index, keep }` 列表。 |
| `review_report` 或 `review_reports[]` | 2c 产出：占位或真实 `verdict` / `coverage_issues` / `completeness_issues` / `token_spent` 等；多轮时可用列表 append-only。 |
| `overrides[]` | 2d 人工 delta：如 `{ index, keep }` 仅记录**被改动的句**；append-only 语义可与「最新有效 overrides」策略结合（由实现约定）。 |
| `keep_mask_final[]` | **定稿** mask：由 `keep_mask` 与 `overrides` 合并得到的、对执行层与下一轮智能层输入一致的有效决策；**必须物化**于快照，以便在裁剪历史后仍能直接驱动 Layer 3 或作为下一轮上下文。 |

**Layer 3 消费**：读取清单顶层 `annotations[]`（时间轴）+ `current.keep_mask_final[]`（或与 `previous` 比较时的目标快照，由编排约定）；**不在清单内持久化** GOP 级剪切中间结果；成片路径可写入 `layer_status` 或扩展字段。

### 11.4 派生物与「不必重复存储」的边界

| 数据 | 是否写入清单 | 理由 |
|------|----------------|------|
| `tokens[]`（句面投影） | **否** | 与 `annotations[]` 句面一一对应的形式投影；随时 `tokens = f(annotations)`。 |
| `cleaned_annotations[]` | **是（在快照内）** | 多轮 + 历史裁剪后，推导链可能断裂；作为 checkpoint 与 2b Prompt 的稳定输入。 |
| `keep_mask_final[]` | **是（在快照内）** | 同上；且为 L3 与跨轮语义的一致锚点。 |
| `edl[]`（时间区间） | **可选** | MVP 已在执行层内部合成；若完整版写入清单，仍可与 `keep_mask_final` 并存，由实现约定单一真相源（SSOT）。 |

### 11.5 循环推进与历史裁剪

智能层存在 **2a→2b→2c→2d→（反馈）→新 2a** 等多轮闭环时（见 §4 子管道图），推荐**磁盘策略**：

1. **轮次结束**：将现有 `current` **整体复制**到 `previous`（覆盖旧 `previous`）。
2. **摘要归档**：将原 `previous`（即刚被挤出的那一轮完整快照）**压缩为一条**记录，append 到 `history_summary[]`；摘要字段可含：`round`、`purpose` 摘要、`keep_rate`、`human_feedback_text`、`timestamp` 等，**不含**全量 `char_timestamps` 或全句正文。
3. **写入新 `current`**：本轮新产生的 `comprehension` / `cleaned_annotations` / `keep_mask` / `review_*` / `overrides` / `keep_mask_final`。

**后果（刻意接受）**：早于 `previous` 的轮次**无法**从清单 alone 无损重放「从最初 corrections 链累加到当前」；**依赖** `current`（及 `previous`）中的**物化 checkpoint**。这是以有限磁盘换取可追溯与可续跑的设计权衡。

### 11.6 与「只增不改」原则的关系

- **全局 append-only**：`annotations[]`、`history_summary[]`、多轮 `review_reports[]` 等适合只追加。
- **快照内允许覆盖**：`current` / `previous` 作为**有界窗口内的可覆盖槽位**，与「无限追加同一字段」区分；语义上仍可通过「先复制到 `previous` 再写 `current`」保持轮次间可追溯。
- **源句面**：`annotations[].content` 永不因智能层被改写；消歧仅体现在 `cleaned_annotations` 与 `comprehension.corrections`。

### 11.7 编排与续跑（自描述）

目标形态下，**宏观阶段**（L1 / L2 / L3）可由清单字段存在性推断续跑起点（与 CLI `--stages` 可组合：CLI 表达意图，清单做合法性校验）。示例规则：

- 无 `annotations[]` → 从 **L1** 开始。
- 有 `annotations[]`，无 `current.comprehension` → 从 **2a** 开始。
- 有 `current.comprehension`，无 `current.keep_mask` → 从 **2b** 开始。
- 有 `current.keep_mask`，待人工或待 L3 → 依 `layer_status` 与 `keep_mask_final` 是否已写入判定。

（具体状态机以实现与测试为准。）

### 11.8 与历史三 JSON 教具的关系（非本节强制）

- **导出**：若需兼容外部旧工具，可实现「从 manifest **导出** 历史形态的 JSON 片段」；非运行时主路径。
- **现行主路径**：编排与各层以 **`timeline_manifest.json`** 为锚；`tokens[]` 仅在内存或调试导出中出现。

### 11.9 MVP-mini（透传 2c/2d 的缩略形态，供对照）

在**无智能层多轮循环**、**2c 占位透传、2d 无人工覆盖**的缩略形态下，可省略 `previous` 与 `history_summary`；`current` 内可不持久化 `review_report` / `overrides`；`keep_mask_final` 可由 `keep_mask` 直接复制写入。该缩略形态用于降低首版实现复杂度，**不替代**本节完整版在循环与审计上的字段语义。
