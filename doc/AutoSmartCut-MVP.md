...
| 2c 审核 | LLM 结构化审核 | 占位：控制台提示 + `review_report`（单条 pass）或等价 | **透传**：可不持久化 `review_report`；逻辑上等价 pass |
| 2d 人工 | CLI / GUI | **默认跳过**（`ascut` 不传 `--interactive-2d`）；可选 CLI 覆盖 | **透传**：`overrides` 不存或恒空；**`keep_mask_final := keep_mask`** |
| 循环 | Token 预算 + 内外环 | **完全禁用** | 仍禁用；无 `previous` / `history_summary` 需求 |

**MVP-mini 与现行 MVP 的差异一句话**：在**不增加 LLM 调用**的前提下，把 2c/2d 收窄为无状态透传，并把**持久化形态**从三 JSON 迁向**单一 `timeline_manifest.json`**（字段级细节见 [AutoSmartCut.md §11.9](AutoSmartCut.md#119-mvp-mini透传-2c2d-的缩略形态供对照)）。
...
| D13 | 智能层 LLM 调用策略 | 2a **两次 LLM**（R1 单轮；R2 与 R1 **同一 `messages` 前缀上的第二跳**，见 `intelligence_llm.prepare_next_turn_messages` + `call_turn_structured`）+ **程序一步**生成 `cleaned_annotations`；2b **单轮** `call_llm_structured`；MVP 共计 **3 次 LLM 调用**；2c 仍为 auto-pass | 2a 中间结构不落盘；持久化 `comprehension` 仅 `purpose` / `outline_blocks` / `cleaned_annotations`；2b 读 `comprehension` + 内存 **`tokens[]`**（由 `annotation_tokens.tokens_from_annotations` 自 `annotations[]` 派生，**不落盘**）；与 [intelligence-layer2-mvp.md](intelligence-layer2-mvp.md) §4–§6、§5.1.1 一致 |
...
- **智能层现行实现**使用内存 **`manifest_dict: dict`**（见 `autosmartcut/intelligence.py`）：自磁盘 **`timeline_manifest.json`** 经 `load_manifest` 后组装键 **`tokens`**（句面，**不落盘**）、`goal`、`source`、`comprehension`、`keep_mask`、`review_report`（单条占位）、`human_feedback_history` 等；**时间轴**来自清单 **`annotations[]`**。**不与** `TimelineManifest` dataclass 做往返转换。
...
**约束（与现行清单一致）：** 智能层写回 **`current.keep_mask`**。`len(keep_mask) == len(annotations)`（与内存 `tokens` 等长）。每条 `keep` **仅**为 `true` 或 `false`（**不使用** `null`）。句间间隔由 **`annotations[].gap_after`** 表达。Layer 3 将 `keep_mask` 与 `annotations[]` 对齐后合并保留段：每段右边界为 **该段最后一句** 的 `t_end + min(gap_after, gap_after_cap)`（默认见 `config.toml` 的 `[execution]`）；再经 `pre_pad` / `post_pad`；**可选**经 **VAD 切点吸附（Snap）** 微调入/出点；再经 `min_duration` 合并等后处理。
