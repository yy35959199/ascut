...
| D18 | 保留段尾部静音上限 | 合并后每段右边界为末句 `t_end + min(gap_after, gap_after_cap)`；`gap_after_cap` 默认 `0.6`，由 `config.toml` 的 `[execution]` 与 `ExecutionConfig` 读取 | 限制长停顿进入成片；`positive_segments_from_mask_files` 未传 `gap_after_cap` 时用 `load_config().execution.gap_after_cap` |
...
class CleanedAnnotation:
    ...
    落盘为 **稠密全量**列表，与 `annotations[]` 等长、按 `index` 对齐；未纠错的句子
    `cleaned_content` 与 `annotations[].content` 相同。不修改原始 annotations[].content。"""
...
- **期望 JSON（LLM）：** `purpose_rough`；`outline_blocks_rough[]`（`start_index`/`end_index`/`topic`，可选）；`candidate_misrecognitions[]`（`annotation_index`、`wrong`、`suggestions[]`）。Schema 必填：`purpose_rough`、`candidate_misrecognitions`。
...
- **期望 JSON（LLM）：** `purpose`、`outline_blocks[]`（`start_index`/`end_index`/`summary`）、`corrections[]`（`index`、`old`、`nth` 1-based、`new`）。
- **验证项：** `corrections` 与 R1 候选及句内子串一致；分块 `summary` 可供 2b 使用。

#### 2a 程序步骤（写入 `comprehension`）

- 根据 `corrections` 在 **`annotations[].content` 副本**上按 `nth` 锚定 `old`→`new` 替换；内部可先产出变化集，再 **回填为稠密全量** `cleaned_annotations[]`（与 `annotations[]` 等长）；**不**改写 Layer1 已落盘原文。
- **验证项：** 稠密列表长度与 index 对齐；有误识的 index 上 `cleaned_content` 与原文差异符合预期。
...
│   ├── config.py                       ← 全局配置（LLM、perception、`[execution].gap_after_cap` 等）
...
> **【给后续协作者的提示】** 下文 §6 仍沿用历史「Stage 1–4」编号，便于对照早期 Demo。**与当前仓库一致的契约**（JSON1/JSON2/JSON3、`keep_mask`、稠密 `cleaned_annotations`、Layer 3 的 `gap_after_cap` 与 `resolve_keep_flags` 等）以本文 **§1–§5**、[intelligence-layer2-mvp.md](intelligence-layer2-mvp.md)（含 §10.1）及 `autosmartcut/execution.py` 为准；§6 中与「稀疏 cleaned_annotations」「以清单 `edl[]` 为主输入」等旧叙述冲突时，以上述为准。
