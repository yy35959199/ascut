...
**覆盖子阶段：** 2a 理解（两轮）、2b 决策（输出 keep_mask）。2c auto-pass 和 2d CLI 审阅不在本 Demo 范围内，留待 MVP 工程化阶段实现。

**当前状态：** Demo 1 与 Demo 3 已完成，JSON2 输入文件（`outputs/demo2_input.json`）已就绪，前置条件满足。

**输入：** Demo 1 产出的 `outputs/demo2_input.json`（JSON2 格式，含 `tokens[]` 索引序列，已去除 char_timestamps）。

#### 2a Round 1 验证（bootstrap）

**Prompt 结构：**
- 系统提示：角色设定为视频内容分析助手，任务是从原始 ASR 转写中提炼粗糙主旨，并识别可能存在误识的专有名词/人名/术语
- 用户消息：JSON2 的 `tokens[]` 文本列表（`index + text`，静音条目跳过）
- 期望 JSON 输出：
...
**Prompt 结构：**
- 系统提示追加 Round 1 产出的 `purpose` 和 `symbol_table[]` 作为上下文
- 用户消息：同 Round 1（相同 tokens 列表）
- 期望 JSON 输出：
...
**验证项：**
- `cleaned_annotations[]` 是否对含误识的 speech 条目都产出了消歧版本
...
**Prompt 结构：**
- 系统提示：注入 `purpose` + `checklist[]`，要求对每条 selectable token 输出 keep 决策
- 用户消息：`cleaned_annotations[]`（消歧后文本，仅 speech 条目）+ 全部 token index 列表
- 期望 JSON 输出（JSON3 格式）：
...
**约束：** `keep_mask` 长度必须等于 JSON2 `tokens[]` 长度；speech 条目 `keep=true/false`，silence 条目 `keep=null`。

**验证项：**
- `keep_mask` 长度与 JSON2 tokens 数量一致（无遗漏 index）
- `checklist_coverage` 中所有 must 项 `covered=true`
- 人工目视：keep=true 的片段是否语义合理
...
| keep_mask 完整性 | 长度 == JSON2 tokens 数，无缺失 index |
| must 项覆盖率 | ≥ 1 次调用中所有 must 项 covered=true |
| 消歧有效性 | cleaned_annotations 中专有名词被纠正（人工目视）|
| 反馈历史压缩 | 第 2 轮调用时，Round 1 结果正确传入 Round 2 上下文 |

#### Go/No-Go

- **Go：** 三轮调用 JSON 解析率 100%，keep_mask 无遗漏，must 项覆盖率达标 → 三轮 Prompt 结构定形，直接复用到 MVP `intelligence.py`。
- **Pivot：** JSON 偶发格式错误 → 加入 retry + schema 长度校验兜底，不阻塞 MVP。
- **Stop：** `cleaned_annotations[]` 完全无效（消歧等于原文）→ 重新设计 Round 2 Prompt；或 keep_mask 系统性偏短（大量漏判）→ 重新设计 2b Prompt 中的 checklist 注入方式。

#### 对后续决策的影响

- 三轮 Prompt 结构在此处定形，直接复用到 MVP `intelligence.py`。
- 2b 输出的 JSON3 文件（`outputs/layer2_output.json`）可直接用于 Demo 3 的 json 模式做端到端验证。
- 若出现长文本超出上下文窗口，此阶段确定分块边界策略（按 `checklist` 段落划分 vs 固定 token 数滑窗）。
