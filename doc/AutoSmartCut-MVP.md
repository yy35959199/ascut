...
- **全链路 MVP（规划目标）**：由 `runner.py` 统一编排 Layer1→2→3，提供 `autosmartcut run` / `--resume` / `--stage` 等；配合 **`checkpoint.py`** 与视频同目录下的 **`.autosmartcut_<视频主文件名>/`**（内放 `manifest.json`、`manifest.layer1.json`、`manifest.layer2.r*.json` 等）做断点续跑与轮次快照。上述为**设计契约**，**当前仓库尚未实现**。
- **现行可用路径（手工串联）**：
  1. Layer 1：`demos/demo1_asr.py`（或等价流程）产出 **JSON1**（`layer1_annotations.json`）；
  2. Layer 2：`python -m autosmartcut.intelligence <layer1.json> <layer2.json> [--goal "..."]` 产出 **JSON3**（仅含 `keep_mask` 等，见下文）；
...
- `runner.py` 统一编排与 **`autosmartcut run`** 式 CLI
- `checkpoint.py` 与 **`.autosmartcut_<视频名>/`** 检查点目录及 `--resume` / `--stage`
...
- **智能层现行实现**使用内存 **`manifest_dict: dict`**（见 `autosmartcut/intelligence.py`）：键包括 `annotations`、`goal`、`source`、`comprehension`、`keep_mask`、`review_report`（单条占位）、`human_feedback_history` 等；**不与** `TimelineManifest` dataclass 做往返转换。
...
- **`comprehension`（MVP，dict）**：`purpose`、`outline_blocks[]`（`start_index`/`end_index`/`summary`）、`cleaned_annotations[]`（**稠密全量**，与 `annotations` 等长；程序按 R2 `corrections` 生成，见 layer2 MVP §5）。
- **`keep_mask`（MVP）**：与 `annotations` 等长，`{"index", "keep"}`，`keep` 仅为布尔；智能层对外交付物；**执行层**用 JSON1+JSON3 在内部合成时间区间，**不要求**清单内 `edl[]`（完整版叙事见 `EditDecision` 注释）。
...
**约束：** `save_layer2_json` 当前仅写出 **`keep_mask`**（可与 JSON1 放在同一目录供 Layer 3 引用；`source` 由 Layer 3 从 JSON1 读取）。`len(keep_mask) == len(JSON1.annotations)`；每条 `keep` **仅**为 `true` 或 `false`（**不使用** `null`）。句间间隔由 JSON1 的 `gap_after` 表达。Layer 3 将 JSON3 与 JSON1 对齐后合并保留段：每段右边界为 **该段最后一句** 的 `t_end + min(gap_after, gap_after_cap)`（默认见 `config.toml` 的 `[execution]`）；再经 `pre_pad` / `post_pad` / `min_duration` 等后处理。
...
| Demo 2 | 智能层（2a→2b→2c 占位→2d） | ⚠️ `demos/demo2_llm.py` 仅占位；**能力在** `python -m autosmartcut.intelligence` + `intelligence_*.py` |
...
Demo 1 与 Demo 3 可并行验证；端到端冒烟：JSON1 → `intelligence` 模块 → JSON3 → `demo3_smartcut.py json`。
...
│   ├── demo2_llm.py              ← 占位；智能层请用 `python -m autosmartcut.intelligence`
...
│   ├── execution.py              ← Layer 3：JSON1+JSON3 → 区间 → Fraction → smartcut
│   ├── intelligence.py           ← Layer 2 编排：load/save JSON，串联 2a–2d
│   ├── intelligence_2a.py
│   ├── intelligence_2b.py
│   ├── intelligence_2c.py        ← MVP 占位 pass
│   ├── intelligence_2d.py        ← 2d CLI（t / a / q）
│   ├── intelligence_llm.py       ← OpenAI 兼容 + jsonschema 校验
│   └── runner.py                 ← 规划中的统一 CLI 入口（当前占位）
├── doc/
│   ├── AutoSmartCut.md
│   ├── AutoSmartCut-MVP.md
...
**说明：** 无 `stages/` 子包、无 `checkpoint.py`（均为规划项）。Qwen3-ASR 可通过 `qwen-asr`（PyPI）或本地 editable 安装，以实际环境为准。
...
| L2 | `intelligence.py`、`intelligence_2a/b/c/d.py`、`intelligence_llm.py` | `manifest_dict`；2a R1+R2+程序 `cleaned_annotations`；2b `keep_mask`；2c 占位；2d 文本 CLI |
| L3 | `execution.py`、`demo3_smartcut.py` | `keep_mask_to_positive_segments` / `positive_segments_from_mask_files`；`MediaContainer` + `smart_cut` |
...
- Layer 2：`python -m autosmartcut.intelligence <layer1.json> <layer2.json> [--goal "..."]`
- Layer 1 / 3：见 `demos/demo1_asr.py`、`demos/demo3_smartcut.py` 的参数说明

**规划中（未实现，需求保留）**

- `autosmartcut run` / `--resume` / `--stage` / `--auto` / `inspect` / `config` 子命令（依赖 `runner.py` + `checkpoint.py`）
- 入口脚本名 `ascut`（`pyproject.toml` 已指向 `runner:main`，待 runner 实现后生效）
...
*文档版本：0.2.0*  
*修订日期：2026-04-10（对齐现行 ascut 实现：dict 智能层、无 runner/checkpoint、L3 走 keep_mask）*  
