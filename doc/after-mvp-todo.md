...
**输入**：`outline_blocks_rough`，`n = len(annotations)`（与 Layer1 连续 `index` 约定一致）。
...
- **与普通 `call_llm_structured` 重试关系**：底层 JSON/网络重试可保留；上述「带错重试」是 **业务层多一轮**、且 **仅一轮**。

### 4. 无 `annotations` 禁止进入 2b

- 在 `run_2b_decision` 入口（或更早在 `run_intelligence_layer`）：`len(annotations)==0` 时 **直接拒绝**（`raise`），不调用 LLM、不写 `keep_mask`。
