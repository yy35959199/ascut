...
| 消歧标注 | Cleaned Annotations | 在不修改原始 `annotations[].content` 的前提下写入清单的校正文本视图。**MVP**：由 **程序**根据 2a R2 的 `corrections` 生成，非 LLM 直接输出整表 |
...
> **MVP 现行（工程契约）**：当前仓库实现的清单字段、层间 JSON（JSON1/JSON2/JSON3）、2a（两轮 LLM + 程序生成 `cleaned_annotations`）、智能层交付 **`keep_mask`**（**不**在智能层落盘 `edl[]`，由执行层合成剪切区间）等，以 [intelligence-layer2-mvp.md](intelligence-layer2-mvp.md) 与 [AutoSmartCut-MVP.md](AutoSmartCut-MVP.md) 为准。  
...
三层管道按时间轴语义逐级收敛、单向流转：**识别层（Layer 1）**负责把原始媒体转为结构化标注，完成音频提取、转写、字级对齐、句级聚合，并在每条句级标注上记录 **`gap_after`**（句间间隔），产出可计算的 `annotations[]`；**智能层（Layer 2）**是唯一与 LLM 交互的中枢，按「2a 理解 → 2b 决策 → 2c 审核 → 2d 人工」闭环运行（完整版含检查清单、片段级覆盖报告与清单内 EDL 等叙事）。**MVP 收窄**：2a 为两轮 LLM + 程序替换生成 `cleaned_annotations`（不持久化符号表）；2b 产出与句级索引对齐的 **`keep_mask`**；**执行层**根据 `keep_mask` 与 JSON1 时间信息**内部**得到剪切区间并驱动 smartcut，详见 [intelligence-layer2-mvp.md](intelligence-layer2-mvp.md)。**执行层（Layer 3）**按 GOP（Group of Pictures，画面组）级精确剪切策略完成输出，绝大多数区间直接搬运原始比特流，仅在切点邻域局部重编码，从而兼顾速度、精度与画质。
...
> **MVP**：无内外循环、无 checklist 主约束；2a 为 R1+R2+程序 `cleaned_annotations`；2d 定稿 `keep_mask`。下图描述**完整版愿景**。
...
- **第 2 轮（印证）**：输入原始文本 + 粗糙主旨 + 符号表，产出精化主旨、`cleaned_annotations[]`（**MVP 中改为程序根据 `corrections` 生成**）与最终 `checklist[]`。其中消歧文本只追加写入，不覆盖 Layer 1 的原始 `annotations[].content`，因此仍然遵循 Append-only。
...
**LLM 做什么（MVP）：** 对每个句级 `index` 输出 **`keep: true|false`**，写入 **`keep_mask[]`**（与 `annotations[]` 等长）；可选预留 `checklist_coverage[]`（可空）。
...
> 以**时间轴清单（TimelineManifest）**为核心资产，分三层单向流转：**识别层**把原始媒体转化为标注，**智能层**通过四子阶段（理解→决策→审核→人工）把标注转化为决策与语义理解（完整版含循环与清单内 EDL 叙事），**执行层**把保留区间渲染为最终产物。**MVP** 下智能层对外为 **`keep_mask`**，剪切区间在执行层合成。每层产物只增不改、可持久化、可重跑；扩展能力通过节点注册实现，不修改主干。视频剪辑是第一个应用，架构的射程是一切时间轴媒体的语义化。
