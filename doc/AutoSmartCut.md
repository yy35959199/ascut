...
| `layer_status` | 各宏观层完成时间戳等元信息（如 `l1_completed_at`、`l2_completed_at`、`l3_completed_at`），便于断点续跑与观测。 |
